...
\begin{remark}
    ...
    Thus, \([\tilde y\circ\sus^2]\in[\sus^{n-k+2}X\big/A,\sus^2 E_n]\cong[\sus^{n-k}X\big/A,E_n]\) by theorem\ \ref{freudenthal}.\\
    We define the boundary map as \(\partial([x])=\tilde y\).
\end{remark}
...
\begin{lemma}[Long exact sequence {\cite[Proposition \(1\cdot 1\)]{atiyah}}]
    For a finite CW-complex \(X\), and subcomplexes \(Z\subset Y\subset X\), we have the long exact sequences
    \[\cdots\to\N^n(X,Y)\to\N^n(X,Z)\to\N^n(Y,Z)\to\N^{n+1}(X,Y)\to\cdots\]
    \[\cdots\to\Omega^n(X,Y)\to\Omega^n(X,Z)\to\Omega^n(Y,Z)\to\Omega^{n+1}(X,Y)\to\cdots\]
\end{lemma}
