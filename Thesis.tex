...
\begin{proof}\cite{atiyah}, \cite{conner}
    ...
    The proof is exactly the same as in the unoriented case; noting that the cylinder is now oriented and \(\partial M\times I\) is now \(\partial I\times (M\cup M^-)=M\amalg M^-\)%\((\partial I\times M)\cup (I\times M^-)\).
\end{proof}
