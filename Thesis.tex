...
Diese Bachelorarbei besch\"aftigt sich mit der Entwicklung der Bordismus-Homologie und -Kohomologie. Daf\"ur werden in Kapitel\ \ref{Kap Manifolds}zun\"achst die ben\"otigten Grundlagen der Differentialtopologie gelegt. 
...
Bordism theory starts with an equivalence relation on manifolds, where two manifolds are called bordant if their disjoint union is the boundary of a higher-dimensional manifold. First, this seems like a very abstract definition, but it turns out to be a very useful one for algebraic topology.\\
...
Bordism groups take a slightly different approach: Instead of spheres, one considers arbitrary compact manifolds and two maps are considered equivalent if there exists a bordism between the manifolds making the maps compatible. This makes computation a lot easier.\\
...
Before we can define bordism, we need some preliminary definitions. We will start with the recalling the definition of manifolds. We will also introduce some useful tools from differential topology.
...
\begin{theorem}[Boundaries are submanifolds {\cite[Theorem 5.12]{lee}}]\label{boundary manifold}
...
\begin{theorem}[Collar theorem\ {\cite[I, Satz 1.5]{brocker}},\ {\cite[Theorem 9.25]{lee}}]\label{collar}
...
\begin{proof}
    ...
    %\todo{See\ \cite[p.223]{lee} for a proof. Maybe I want Br\"ocker, tom Dieck here}
\end{proof}
...
\begin{proposition}[{\cite[Proposition 5.47a, Corollary 5.14]{lee}}]
...
\begin{definition}[Measure zero {\cite[p.126]{lee}}]
    A subset \(S\subset\R^n\) has \demph{measure zero} if, for any \(\varepsilon>0\), there exists a countable collection of rectangles
    ...
    A subset \(S\) of a manifold \(M\) is said to have \demph{measure zero}, if for every chart \((U,\varphi)\) in its atlas, the set \(\varphi(S\cap U)\) has measure zero in \(\R^n\).
\end{definition}
...
\begin{definition}[Connected sum {\cite[Example 9.31]{lee}}]
    For two connected manifolds \(M^n,N^n\), choose smooth embeddings \(D^n\xhookrightarrow{i}\mathrm{int}(M)\), \(D^n\xhookrightarrow{j}\mathrm{int}(N)\). Then the connected sum of \(M\) and \(N\) is defined as the pushout %\todo{could be done as pushout}
    %\[M\#N:=((M\setminus i(\overset{\circ}{D^n}))\dis(N\setminus j(\overset{\circ}{D^n})))\big/(i(x)\sim j(x))\]
    %for all \(x\in\partial D^n\).
    \begin{center}
        \begin{tikzcd}
            \partial D^n \arrow[d, "j\restrict{\partial D^n}"] \arrow[r, "i\restrict{\partial D^n}"] \arrow[dr, phantom, "\ulcorner", near end] & M\setminus i(\overset{\circ}{D^n}) \arrow[d, " "] \\
            N\setminus j(\overset{\circ}{D^n}) \arrow[r, " "] & M\#N
        \end{tikzcd}
    \end{center}
    \begin{center}
...
\subsubsection{Relative Bordism}\label{Kap rel. Bordism}
...
\begin{definition}[Relative bordism\ {\cite[pp.524-525]{dieck}}]
    For a pair of topological spaces \((X,A)\), we call \((M,f)=(M,\partial M,f)\) a \demph{singular manifold in }\((X,A)\) if \((M,f)\) is a singular manifold in \(X\) and \(f:(M,\partial M)\to (X,A)\) is a continuous map of pairs.\\
...
\begin{lemma}[Functoriality\ {\cite[II, Satz 3.2]{brocker}}]\label{functoriality}
...
\begin{definition}[Oriented manifold\ {\cite[\nopp VIII.8]{tomdieck}}]\label{oriented manifold}%{\cite[p.380]{lee}}]\
...
\section{Cobordism}\label{Kap Cobordism}
...
\subsubsection{(Thom) Spectra}\label{Kap Spectra}
...
\begin{definition}[Thom space {\cite[p.29]{thom}}]%\cite{brocker}, {\cite[p.29]{thom}}, {\cite[p.201]{atiyah}}]
...
\begin{example}[Suspension spectrum {\cite[\nopp 7.7.2]{dieck}}]\label{sus spectrum}
    Given a pointed space \(X\), we can define its \demph{suspension spectrum} \(\sus^\infty X\) by \((\sus^\infty X)_n=\sus^n X\) and the structure maps as the identities for \(n\geq0\). For negative \(n\), we take \((\sus^\infty X)_n=\{\pt\}\) and the structure maps as the identity map \(\sigma_n:\sus(\{\pt\})=\{\pt\}\to\{\pt\}\) except for \(n=-1\), where we take the inclusion map to the basepoint.
    Note that \((\sus^\infty X)_0=X\)
\end{example}

\begin{example}[Sphere spectrum {\cite[\nopp 7.7.1]{dieck}}]
    Taking \(X=S^0\) if example\ \ref{sus spectrum} gives us the \demph{sphere spectrum} \(\underline{S}\). Then, \(S_n=S^n\) for non-negative \(n\).
\end{example}
...
\begin{theodef}[Cohomology theory associated to a spectrum {\cite{whitehead}}]\label{spectrum cohomology}
...
\begin{example}[Stable cohomotopy groups {\cite[\nopp 7.7.1]{dieck}}]\label{stable cohomotopy groups}
    For the sphere spectrum \(\underline{S}\), we get the \demph{stable cohomotopy groups} via the above definition:
...
\begin{lemma}[Long exact sequence {\cite[Proposition \(1\cdot 1\)]{atiyah}}]\label{coles}
...
\begin{lemma}[Disjoint union axiom]\label{codisjoint}
...
\begin{remark}[Dimension axiom]\cite{thom}
...
%We will show that this is indeed the case, and that it is a consequence of Brown's representation theorem\ \cite[Theorem 9.27]{switzer}. theoreme IV.8 in thom vllt

\begin{theorem}[Brown's representation theorem {\cite[Theorem 9.27]{switzer}}, {\cite{brown}}]%{\cite[\s 4 Theorem II]{brown}}]
...
%\begin{proof}
%    See\ \cite{switzer} or\ \cite{brown}.
%\end{proof}
...
\begin{theodef}[Homology theory assigned to a spectrum {\cite[IV, Definition 1.5]{brocker}}]
...
\begin{example}[Stable homotopy groups {\cite[\nopp 20.9]{dieck}}]
    Taking the sphere spectrum \(\underline{S}\), we get the \demph{stable homotopy groups}:
...
\begin{theorem}[{\cite[Th\'eor\`eme IV.8]{thom}}]
