...
\begin{definition}[\todo{Measure zero}]
    A subset \(A\) of a topological space \(X\) is said to have \demph{measure zero}, if for every \(\varepsilon>0\), there exists a countable collection of open sets \(\{U_i\}_{i\in\mathbb{N}}\) such that \(A\subseteq \bigcup_{i\in\mathbb{N}}U_i\) and \(\sum_{i=1}^\infty \mathrm{vol}(U_i)<\varepsilon\), where \(\mathrm{vol}\) is the volume measure on \(X\).
\end{definition}
...
Thus, we can view the bordism groups as stable homotopy groups:
\begin{corollary}[{\cite[Th\'eor\`eme IV.8]{thom}}]
    The absolute bordism groups \(\N_k\) and \(\Omega_k\) are isomorphic to the stable homotopy groups of the respective Thom spectra:
    \[\N_k\cong\lim_{n\to\infty}\pi_{n+k}(MO)\]
    \[\Omega_k\cong\lim_{n\to\infty}\pi_{n+k}(MSO)\]
\end{corollary}
