...
\begin{definition}[Boundary\ {\cite[p.25]{lee}}]
    Let \(M^n\) be a manifold. A point \(x\in M^n\) is called an \demph{interior point} if it admits a neighbourhood homeomorphic to \(\mathbb{R}^n\). Otherwise, it is called a \demph{boundary point}. The set of boundary points is denoted by \(\partial M\) and is called the \demph{boundary} of \(M\).\\
...
\begin{definition}[Connected sum {\cite[Example 9.31]{lee}}]
    For two connected manifolds \(M^n,N^n\), choose smooth embeddings \(D^n\xhookrightarrow{i}M\), \(D^n\xhookrightarrow{j}N\). Then the connected sum of \(M\) and \(N\) is defined as \todo{could be done as pushout}
