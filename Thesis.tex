...
\begin{theorem}[Collar theorem \ {\cite[I, Satz 1.5]{brocker}}]
...
\begin{proof}\cite{brocker}, \cite[p.8]{conner}
    \begin{itemize}
        ...
        We can find a collar of \(M_2\) in both \(B_1\) and \(B_2\). 
...
\begin{remark}
    The above example is just the same as the cylinder bordism in Example\ \ref{cylinder bordism} (if we had constructed the cylinder we would not have to have worried about the embedding). This cylinder bordism now also works for manifolds with non-empty boundary, we can take its lateral surface (\(=\partial M_0\times [0,1]\)) as \(M'\).
\end{remark}
...
\begin{proof}\cite{tomdieck}
    ...
    Let \(B=M\times [0,1]\) the cylinder.\ \(\partial B= \underbrace{M\times\{0\}}_{M_0}\cup \underbrace{M\times\{1\}}_{M_1}\cup \partial M\times I\). Define \(F:B\to X\) as \(F(p,t)=f(p)\).\\
...
\begin{lemma}[Naturality of the boundary map]
    ...
    \begin{center}
        \begin{tikzcd}
            {\mathfrak{N}_n(X,A)} \arrow[r, "\partial_n"] \arrow[d, "{f\restrict{A}}_\ast"'] & {\mathfrak{N}_{n-1}(A)}  \arrow[d, "f_\ast"] \\
