...
\begin{remark}
    In this thesis, with \demph{manifold} we will always mean a smooth manifold with boundary, unless specified otherwise.
\end{remark}
...
\begin{definition}[Boundary\ {\cite[p.25]{lee}}]
    ...
    If a \(M\) is compact and has empty boundary, \(M\) is called a \demph{closed manifold}. %If a manifold is compact and has non-empty boundary, it is called a \demph{compact manifold with boundary}. If a manifold is not compact, it is called an \demph{open manifold}
\end{definition}
...
\begin{definition}[critical value {\cite[p.105]{lee}}]
    ...
    A point \(q\in N\) is a \demph{critical value} of \(f\), if \(f^{-1}(q)\) containa a critical point of \(f\). Otherwise, it is called a \demph{regular value}.
\end{definition}
...
\begin{remark}
    These examples can be generalized to just \(f\) being a constant %maybe even nullhomotopic
...
\begin{example}[TODO: Pair of pants]
...
Probably to do: separating function
...
\begin{remark}
    If a homology theory satisfies the dimension axiom, it is called an \demph{ordinary homology theory}.
\end{remark}
...
\begin{definition}[relative bordism\ {\cite[pp.524-525]{dieck}}]
    ...
    Two \(n\)-dimensional singular manifolds \((M^n_0,f_0)\) and \((M^n_1,f_1)\) in \(X\) are called \demph{bordant}, if there exists an (\(n+1\))-dimensional singular manifold \((B^{n+1},F)\) in \(X\), such that:
    \begin{itemize}
...
\begin{lemma}\ \cite[Lemma 13.10]{tomdieck} \label{almost excision} %\cite[Lemma 5.1]{zhang} \cite[Lemma 21.1.8]{dieck}
    Let \([M,f]\in\mathfrak{N}_n(X,A)\) and \(N\) a \(n\)-dimensional embedded submanifold of \(M\). 
...
\begin{lemma}[{\cite[Chapter I, 3.1]{conner}}]\label{plemma}%{\cite[Lemma 5.6]{zhang}}
    Let \(M\) be a closed manifold, let \(K,L\subseteq M\) closed, such that \(K\cap L=\emptyset\). Then there exists a closed submanifold with boundary \(N\) of \(M\) such that \(K\subseteq N\) and \(L\cap N=\emptyset\).
\end{lemma}
...
\begin{proof}\cite{conner}
    ...
    Then \(N:=\alpha^{-1}([0,r])=\alpha^{-1}(-\infty,r]\) is a closed submanifold of \(M\) %with boundary \(\partial N=\alpha^{-1}(r)\)
...
\begin{lemma}[{\cite[Lemma 5.8]{zhang}}]\label{plemma2}
    Let \(K,L,M,N\alpha,r\) be as in\ \ref{plemma}. Then,
...
\begin{proof}
    ...
    Again, let \(K=F^{-1}(X\setminus\overset{\circ}A)\), \(L=F^{-1}(\overline{Z})\). By lemma\ \ref{plemma}, we have a submanifold \(N\subseteq B\) such that \(K\subseteq N\), \(N\cap L=\emptyset\). Then, \([\partial N,F\restrict{\partial N}]\in\mathfrak{N}_n(X\setminus Z, A\setminus Z)\) nullbordant by \((N,F\restrict{N})\).\\
...
\begin{proof}\cite{atiyah}, \cite{conner}
    ...
    The proof is exactly the same as in the unoriented case; noting that the cylinder is now oriented and \(\partial M\times I\) is now \(\partial I\times (M\cup M^-)=M\dis M^-\)%\((\partial I\times M)\cup (I\times M^-)\).
\end{proof}
